...
#ifndef __FEM_SUPG_ADV_DIFF_H__
#define __FEM_SUPG_ADV_DIFF_H__
...
template <typename T> class SUPG_ADV_DIFF<FEM, T> : public DifferentialExpr<SUPG_ADV_DIFF<FEM, T>> {
private:
...
public:
    ...
    SUPG_ADV_DIFF() = default;
    explicit SUPG_ADV_DIFF(const T& data) : data_(data) { }
    ...
    template <typename... Args> auto integrate(const std::tuple<Args...>& mem_buffer) const {
        ...
        double normb = std::get<2>(data_);
        double delta = std::get<3>(data_);
