...
void LagrangianBasis<DomainType, order>::enumerate_dofs() {
    ...
    if constexpr (R == 1) {
      ...
      for (auto edge = domain_->facet_begin(); edge != domain_->facet_end(); ++edge) {
            for (int i = 0; i < DomainType::n_elements_per_facet; ++i) {
                ...
                if (element_id >= 0) {
                    ...
                    for (; j < edge_pattern.rows(); ++j) {
                        ...
                        for (int k = 0; k < DomainType::n_vertices_per_edge; ++k) {
      ...
      for (int i = 0, n = locs.rows(); i < n; ++i) {
            ...
            for (int h = 0, n_basis = basis.size(); h < n_basis; ++h) {
                triplet_list.emplace_back(
                  i, dofs(e.ID(), h),
                  basis[h](e.inv_barycentric_matrix() * (p_i - e.coord(0))));   // \psi_j(p_i)
            }
      }
                    ...
                    for (int h = 0, n_basis = basis.size(); h < n_basis; ++h) {
                        triplet_list.emplace_back(
                          ...
                          integrator.integrate(e, [&e, &psi_h = basis[h]](const SVector<N>& q) -> double {
                              return psi_h(e.inv_barycentric_matrix() * (q - e.coord(0)));
                          }));
                        head++;
                    }
