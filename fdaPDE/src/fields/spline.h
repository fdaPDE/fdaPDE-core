...
class Spline : public ScalarFieldBase<1, Spline> {
   ...
   private:
    ...
    class Derivative : public ScalarFieldBase<1, Derivative> {
       ...
       public:
        ...
        constexpr Derivative(const std::vector<double>& knots, int i, int degree, int n) :
            knots_(knots), i_(i), degree_(degree), n_(n) { }

        // non-recursive implementation of degree k-th spline derivative evaluation at point, as detailed in "Piegl, L.,
        ...
        constexpr Scalar operator()(const InputType_& p_) const {
            ...
            if (p < knots_[i_] || p >= knots_[i_ + degree_ + 1]) return 0.0;
            // initialize triangular table for basis functions
            MdArray<double, MdExtents<Dynamic, Dynamic>> N(degree_ + 1, degree_ + 1);
            ...
            for (int j = 0; j <= n_; ++j) { ND[j] = N(j, degree_ - n_); }
            // compute the nth derivative using the table of width n_
            for (int jj = 1; jj <= n_; jj++) {
                double saved = 0.0;
                if (ND[0] != 0.0) { saved = ND[0] / (knots_[i_ + degree_ - n_ + jj] - knots_[i_]); }
                for (int j = 0; j < n_ - jj + 1; j++) {
                    double Uleft  = knots_[i_ + j + 1];
                    double Uright = knots_[i_ + j + 1 + degree_ - n_ + jj];
                    if (ND[j + 1] == 0.0) {
                        ND[j] = (degree_ - n_ + jj) * saved;
                        saved = 0.0;
                    } else {
                        double temp = ND[j + 1] / (Uright - Uleft);
                        ND[j] = (degree_ - n_ + jj) * (saved - temp);
                        saved = temp;
                    }
                }
            }
   ...
   public:
    ...
    Spline(KnotsVectorType&& knots, int i, int degree) : i_(i), degree_(degree) {
      	fdapde_assert(knots.size() >= degree_ + 1 && i_ < knots.size());
    ...
    constexpr Scalar operator()(const InputType_& p_) const {
        ...
        // special cases
        if ((i_ == 0 && p == knots_[0]) || (i_ == m - degree_ - 1 && p == knots_[m])) return 1.0;
        // local property: return 0 if p is outside the range of this basis function
	if (p < knots_[i_] || p >= knots_[i_ + degree_ + 1]) return 0.0;
    ...
    // overload of the call operator for a single double
    constexpr Scalar operator()(double p) const { return operator()(std::vector<double>{p}); }
    constexpr Derivative gradient(int n = 1) const { return Derivative(knots_, i_, degree_, n); }
    const std::vector<double>& knot_vector() const { return knots_; }
    int degree() const { return degree_; }
    int knot_id() const { return i_ + degree_ - 1; }
    double knot() const { return knots_[i_ + degree_ - 1]; }
    constexpr int input_size() const { return StaticInputSize; }
   private:
    std::vector<double> knots_ {};
    int i_ = 0;       // knot index where this basis element is centered
    int degree_ = 0;   // B-spline degree
};
