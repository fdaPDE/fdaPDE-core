...
template <typename Derived_> class Laplacian : public ScalarFieldBase<Derived_::StaticInputSize, Laplacian<Derived_>> {
   public:
    using Derived = Derived_;
    template <typename T> using Meta = Laplacian<T>;
    using Base = ScalarFieldBase<Derived::StaticInputSize, Laplacian<Derived>>;
    using FunctorType =
