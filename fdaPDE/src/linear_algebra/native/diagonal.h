...
template <typename XprType_> struct Diagonal : public MatrixExpr<Diagonal<XprType_>> {
   ...
   public:
    ...
    static constexpr int Rows =
      XprType::Rows == Dynamic || XprType::Cols == Dynamic ? Dynamic : XprType::Rows;
    ...
    constexpr explicit Diagonal(XprType__&& xpr) :
        xpr_(std::forward<XprType__>(xpr)), size_(xpr_.rows() == xpr_.cols() ? xpr_.rows() : 0) {
        fdapde_static_assert(
    ...
    constexpr Diagonal(const Diagonal& other) :
        xpr_(other.xpr_), size_(other.size_) { }   // don't copy as we implicitly point to other's diagonal
    ...
    // non-const access
    constexpr Scalar& operator()(int i, [[maybe_unused]] int j) requires(ReadOnly == 0) {
        fdapde_assert(i >= 0 && i < rows() && j >= 0 && j < cols());
        return xpr_(i, i);
    }
    constexpr Scalar& operator[](int i) requires(ReadOnly == 0) {
        fdapde_assert(i >= 0 && i < rows());
        return xpr_(i, i);
    }
    // observers
...
struct diagonal_wrapper : DiagonalMatrixExpr<diagonal_wrapper<Rows_, Cols_, DiagonalXprType_>> {
   private:
    ...
    using XprTypeNested = fdapde::internals::ref_select_t<DiagonalXprType_>;
   public:
    using Scalar = typename XprType::Scalar;
    ...
    constexpr diagonal_wrapper(XprType__&& xpr) :
        Base(XprType::Rows == 1 ? xpr.cols() : xpr.rows()), xpr_(std::forward<XprType__>(xpr)) { }
...
template <typename XprType_> struct DiagonalMatrixExpr : public MatrixExpr<XprType_> {
    ...
    template <typename RhsXprType> constexpr auto solve(const RhsXprType& b) const {
        ...
        if (b.size() != derived().rows()) {
            fdapde_assert(b.size() == derived().rows());
            return x;
        }
...
class DiagonalMatrix : public DiagonalMatrixExpr<DiagonalMatrix<Scalar_, Rows_>> {
   ...
   public:
    ...
    constexpr DiagonalMatrix(const DiagonalMatrixExpr<RhsXprType_>& rhs) : Base(rhs.rows()) {
        clone_(rhs.derived());
    }
    constexpr explicit DiagonalMatrix(const std::vector<Scalar>& vec) : Base(vec.size()), data_() {
        ...
        if (!std::cmp_equal(data_.size(), vec.size())) {
            fdapde_assert(std::cmp_equal(data_.size() FDAPDE_COMMA vec.size()));
            return;
        }
   ...
   public:
    ...
    constexpr DiagonalMatrixView(Scalar* data, int size) : Base(size), data_(data) {
        if (size < 0 || (size > 0 && data == nullptr)) {
            fdapde_assert(size >= 0 && (size == 0 || data != nullptr));
            this->size_ = 0;
            data_ = nullptr;
        }
    }
    ...
    constexpr VectorView<Scalar, Rows_> diagonal() requires(!std::is_const_v<Scalar>) {
        if constexpr (Rows_ == Dynamic) { return VectorView<Scalar, Rows_>(data_, this->rows()); }
        return VectorView<Scalar, Rows_>(data_);
    }
    // inherit assignment from Base
    using Base::operator=;
    constexpr DiagonalMatrixView& operator=(const DiagonalMatrixView& other) & {
        static_cast<MatrixExpr<DiagonalMatrixView>&>(*this).template operator=<DiagonalMatrixView>(other);
        return *this;
    }
    constexpr DiagonalMatrixView operator=(const DiagonalMatrixView& other) && {
        static_cast<MatrixExpr<DiagonalMatrixView>&>(*this).template operator=<DiagonalMatrixView>(other);
