...
struct ReshapeOp : public MatrixExpr<ReshapeOp<Rows_, Cols_, XprType>> {
    ...
    fdapde_static_assert(
      Rows_ == Dynamic || Cols_ == Dynamic || XprType::Rows == Dynamic || XprType::Cols == Dynamic ||
        Rows_ * Cols_ == XprType::Rows * XprType::Cols,
      INVALID_RESHAPE__DIFFERENT_STATIC_SIZE);
    using Base = MatrixExpr<ReshapeOp<Rows_, Cols_, XprType>>;
    using XprTypeNested = fdapde::internals::ref_select_t<XprType>;
    using Scalar = typename XprType::Scalar;
    static constexpr int Rows = Rows_;
    static constexpr int Cols = Cols_;
    static constexpr int StorageOrder = XprType::StorageOrder;
    static constexpr int NestAsRef = 0;
    static constexpr int ReadOnly = std::is_const_v<std::remove_reference_t<XprType>> || XprType::ReadOnly;
    ...
    template <typename XprType_>
        requires(
          !std::same_as<std::remove_cvref_t<XprType_>, ReshapeOp> &&
          internals::safely_nestable<XprTypeNested, XprType_>)
    ...
    template <typename XprType_>
        requires(internals::safely_nestable<XprTypeNested, XprType_>)
    ...
    template <typename XprType_>
        requires(internals::safely_nestable<XprTypeNested, XprType_>)
    constexpr ReshapeOp(XprType_&& xpr, int rows) : ReshapeOp(std::forward<XprType_>(xpr), rows, 1) {
    ...
    constexpr ReshapeOp& operator=(const ReshapeOp& rhs) & requires(ReadOnly == 0) {
        Base::template operator=<ReshapeOp>(rhs);
        return *this;
    }
    constexpr ReshapeOp operator=(const ReshapeOp& rhs) && requires(ReadOnly == 0) {
    ...
    constexpr decltype(auto) bitpack(int i) const { return std::as_const(xpr_).bitpack(i); }
    constexpr int bitpacks() const { return xpr_.bitpacks(); }
   private:
    constexpr void validate_(bool target_matches = true) {
        const bool valid = target_matches && rows_ >= 0 && cols_ >= 0 &&
          static_cast<long long>(rows_) * cols_ == xpr_.size();
        fdapde_assert(valid);
        if (!valid) {
            rows_ = 0;
            cols_ = 0;
