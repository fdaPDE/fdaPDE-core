...
template <typename Lhs, typename Rhs, bool HasScalar =
...
struct MatrixCoeffWiseOp : public MatrixCoeffWiseExpr<MatrixCoeffWiseOp<XprType_, CoeffOp>> {
   ...
   public:
    ...
    using Scalar = std::remove_cvref_t<std::invoke_result_t<CoeffOp&, InputScalar&>>;
    static constexpr int Rows = XprType::Rows;
    static constexpr int Cols = XprType::Cols;
    static constexpr int StorageOrder = XprType::StorageOrder;
    static constexpr int NestAsRef = 0;
    static constexpr bool Writable = requires(CoeffOp op, XprType& xpr, int i, int j) {
        { xpr(i, j) } -> std::same_as<InputScalar&>;
        { op(xpr(i, j)) } -> std::same_as<InputScalar&>;
    };
    ...
    template <typename Scalar_>
        requires(ReadOnly == 0 && std::is_convertible_v<Scalar_, Scalar>)
...
struct MatrixCoeffWiseBinOp : public MatrixCoeffWiseExpr<MatrixCoeffWiseBinOp<LhsXprType_, RhsXprType_, BinaryOp>> {
   private:
    ...
    using LhsXprTypeNested = fdapde::internals::ref_select_t<LhsXprType_>;
    using RhsXprTypeNested = fdapde::internals::ref_select_t<RhsXprType_>;
   ...
   public:
    ...
    using Scalar = std::remove_cvref_t<
      std::invoke_result_t<BinaryOp&, typename LhsTraits::Scalar, typename RhsTraits::Scalar>>;
...
template <typename XprType_> struct MatrixCoeffWiseExpr {
   ...
   public:
    ...
    template <typename CoeffOp_> constexpr auto apply(CoeffOp_&& op) const {
        ...
        return MatrixCoeffWiseOp<XprType_, StoredOp>(derived(), std::forward<CoeffOp_>(op));
    }
    ...
    constexpr auto exp() const {
        return internals::make_cwise_op(derived(), [](const auto& x) { return internals::exp(x); });
    }
    constexpr auto log() const {
        return internals::make_cwise_op(derived(), [](const auto& x) { return internals::log(x); });
    }
    // unary negation
    ...
    template <typename Scalar_>
        requires(
          XprType::ReadOnly == 0 && std::is_arithmetic_v<Scalar_> &&
    ...
    constexpr XprType& operator+=(const MatrixCoeffWiseExpr<RhsXprType_>& rhs) & {
        ...
        using RhsXprType = std::decay_t<RhsXprType_>;
        Matrix<typename RhsXprType::Scalar, RhsXprType::Rows, RhsXprType::Cols, RhsXprType::StorageOrder> tmp(
          rhs.mwise());
