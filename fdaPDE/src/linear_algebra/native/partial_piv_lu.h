...
#ifndef __FDAPDE_LINALG_NATIVE_PARTIAL_PIV_LU_H__
#define __FDAPDE_LINALG_NATIVE_PARTIAL_PIV_LU_H__
...
template <typename XprType_> class PartialPivLU {
   ...
   public:
    ...
    template <typename MatrixType> constexpr void compute(const MatrixExpr<MatrixType>& matrix) {
        ...
        for (int k = 0; k < n; ++k) {
            ...
            if (!(max_value > threshold)) {
                if (info_ == 0) info_ = k + 1;   // LAPACK convention: first unusable pivot, one based
            } else {
                ++rank_;
            }
    ...
    // U is reported in the input scale. If an exact factor exceeds Scalar's
    // range it can overflow, while solve() remains normalized internally.
    ...
    // determinant() can remain available after representational factor growth.
    ...
    constexpr Scalar determinant() const {
        if (!determinant_computed_) {
            fdapde_assert(determinant_computed_);
            return Scalar(0);
        }
        return determinant_;
    }
    ...
    template <typename RhsType> constexpr auto solve(const MatrixExpr<RhsType>& rhs) const {
        ...
        solution.set_zero();
        const bool valid = info_ == 0 && rhs.rows() == L_.rows() && rhs.cols() > 0;
        if (!valid) {
            fdapde_assert(valid);
            return solution;
        }
   ...
   private:
    ...
    // Exponent-tracked elimination prevents intermediate overflow or underflow
    // from changing a representable determinant.
    template <typename MatrixType> static constexpr Scalar algebraic_determinant_(const MatrixType& matrix) {
        ...
        for (int col = 0; col < n; ++col) {
            ...
            const ScaledValue pivot_value = work[index(col, col)];
            determinant = multiply_(determinant, pivot_value);
            for (int row = col + 1; row < n; ++row) {
                const ScaledValue multiplier = divide_(work[index(row, col)], pivot_value);
...
#endif   // __FDAPDE_LINALG_NATIVE_PARTIAL_PIV_LU_H__
