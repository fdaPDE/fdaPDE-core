...
struct partial_matrix_redux_op : public MatrixExpr<partial_matrix_redux_op<XprType, ReductionOp, ByRow>> {
   private:
    ...
    using XprTypeNested = fdapde::internals::ref_select_t<std::conditional_t<XprType::ReadOnly, const XprType, XprType>>;
   public:
...
struct MatrixVectorWiseOp : public MatrixExpr<MatrixVectorWiseOp<XprType, ByRow>> {
   ...
   public:
    ...
    template <typename XprType_>
        requires(
          !std::same_as<std::remove_cvref_t<XprType_>, MatrixVectorWiseOp> &&
    ...
    template <typename XprType_>
        requires(ReadOnly == 0)
    constexpr MatrixVectorWiseOp& operator=(const MatrixExpr<XprType_>& rhs) & {
        using RhsXprType = std::decay_t<XprType_>;
   ...
   private:
    ...
    constexpr void partial_redux_inplace_loop_(const MatrixExpr<XprType_>& rhs, Operator_&& op) {
        ...
        const bool compatible =
          (Rows == 1 && xpr_.cols() == rhs.cols()) || (Cols == 1 && xpr_.rows() == rhs.rows());
