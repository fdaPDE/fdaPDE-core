...
#ifndef __FDAPDE_LINALG_NATIVE_GMRES_H__
#define __FDAPDE_LINALG_NATIVE_GMRES_H__
...
template <typename XprType_, typename Preconditioner_> class GMRES {
   ...
   public:
    ...
    constexpr GMRES(Preconditioner&& preconditioner, int max_iterations, int restart, Scalar tolerance) :
        ...
        parameters_valid_ = max_iterations_ > 0 && restart_ > 0 && tolerance_ > Scalar(0) && std::isfinite(tolerance_);
        if (!parameters_valid_) fdapde_assert(parameters_valid_);
    }
    ...
    constexpr void compute(const MatrixExpr<MatrixType>& matrix) {
        ...
        const bool shape_valid = parameters_valid_ && n > 0 && n == matrix.cols() && (Rows == Dynamic || n == Rows) &&
                                 (Cols == Dynamic || n == Cols);
        if (!shape_valid) {
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                if (!std::isfinite(static_cast<Scalar>(matrix.derived()(i, j)))) {
                    fdapde_assert(std::isfinite(static_cast<Scalar>(matrix.derived()(i FDAPDE_COMMA j))));
                    return;
                }
            }
        }
        ...
        if (!preconditioner_valid_()) {
            fdapde_assert(preconditioner_valid_());
            return;
        }
    ...
    constexpr auto solve(const MatrixExpr<RhsType>& rhs, const MatrixExpr<InitialType>& initial) {
        ...
        Vector<Scalar, Rows> solution;
        if constexpr (Rows == Dynamic) solution.resize(initial.rows());
        const bool shape_valid = initialized_ && rhs.cols() == 1 && initial.cols() == 1 &&
                                 rhs.rows() == matrix_.rows() && initial.rows() == matrix_.rows();
        bool coefficients_valid = shape_valid;
        if (shape_valid) {
            for (int row = 0; row < matrix_.rows(); ++row) {
                coefficients_valid = coefficients_valid && std::isfinite(static_cast<Scalar>(rhs.derived()(row, 0))) &&
                                     std::isfinite(static_cast<Scalar>(initial.derived()(row, 0)));
            }
        }
        const bool input_valid = shape_valid && coefficients_valid;
        if (!input_valid) {
            fdapde_assert(input_valid);
            if (initial.cols() == 1 && solution.rows() == initial.rows())
                solution = initial;
            else
                solution.set_zero();
            reset_observers_();
...
GMRES(const MatrixExpr<XprType>&, Preconditioner&&, int, int, typename XprType::Scalar)
...
#endif   // __FDAPDE_LINALG_NATIVE_GMRES_H__
