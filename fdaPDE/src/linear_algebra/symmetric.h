   ...
   public:
    ...
    constexpr symmetric_wrapper(XprType__&& xpr) :
        ...
        if (xpr_.rows() <= 0 || xpr_.rows() != xpr_.cols()) {
            throw std::invalid_argument("symmetric view requires positive square dimensions");
        }
    }
...
class SymmetricMatrixView :
   ...
   public:
    ...
    using Base::operator=;
    constexpr SymmetricMatrixView& operator=(const SymmetricMatrixView& other) & requires(ReadOnly == 0) {
    ...
    constexpr Scalar_* data() requires(!std::is_const_v<Scalar_>) { return data_.data(); }
   private:
    StorageType data_;
};
