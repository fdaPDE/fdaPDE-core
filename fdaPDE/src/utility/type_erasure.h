...
template <typename T, auto FuncPtr, std::size_t... Is> auto load_method(std::index_sequence<Is...>) {
    using Signature = internals::fn_ptr_traits<FuncPtr>;
...
void init_vtable(value_list<FuncPtr, V, Vs...>, void** vtable) {
    vtable[N] = reinterpret_cast<void*>(
      load_method<T, FuncPtr>(std::make_index_sequence<internals::fn_ptr_traits<FuncPtr>::n_args> {}));
    init_vtable<N + 1, T, V, Vs...>(value_list<V, Vs...>(), vtable);
}
template <int N, typename T, auto FuncPtr> void init_vtable(value_list<FuncPtr>, void** vtable) {   // end of recursion
    vtable[N] = reinterpret_cast<void*>(
      load_method<T, FuncPtr>(std::make_index_sequence<internals::fn_ptr_traits<FuncPtr>::n_args> {}));
}
...
// storage strategies for the owned type-erased object, inspired from boost.te (https://github.com/boost-ext/te)
...
template <typename StorageType, typename... I> class erase : vtable_handler, public I... {
	...
	size_ = (internals::size<typename I::template fn_ptrs<T_>>::value + ...);
	delete[] vtable_;
        vtable_ = new void*[size_];
        internals::init_vtable<0, T_>(typename internals::merge<typename I::template fn_ptrs<T_>...>::type(), vtable_);
        // initialize offset table
        int base_ = 0;
        auto load_offset = [this, &base_](const auto& i) -> void {
            offset_table_[typeid(std::decay_t<decltype(i)>)] = base_;
            base_ += internals::size<typename std::decay<decltype(i)>::type::template fn_ptrs<T_>>::value;
        };
...
template <auto... Vs> using bindings = internals::value_list<Vs...>;

}   // namespace fdapde

#endif   // __FDAPDE_TYPE_ERASURE_H__
