...
// returns a sparse coefficient from a separate translation unit for the link check
int sparse_header_order() {
    const fdapde::SparseMatrix<double> matrix(
      2, 2,
      {
        {1, 1, 4.0}
    });
    return static_cast<int>(matrix.coeff(1, 1));
}
