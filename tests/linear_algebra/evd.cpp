...
concept permits_rvalue_eigenvalues = requires(Decomposition decomposition) {
    std::move(decomposition).eigenvalues();
};

template <typename Decomposition>
concept permits_rvalue_eigenvectors = requires(Decomposition decomposition) {
...
template <int StorageOrder> void check_evd_shapes_lifetime_and_storage_order() {
    ...
    const auto retained_evd = [] {
        const Matrix<double, 3, 3, StorageOrder> source(
          {4.0, 1.0, -2.0, 1.0, 3.0, 0.5, -2.0, 0.5, 2.0});
