...
template <typename T>
concept permits_rvalue_assignment = requires(T value, const fixed_matrix matrix) { std::move(value).assign(matrix); };
template <typename T>
...
template <typename T>
concept exposes_trusted_tag = requires { typename T::trusted_t; };
template <typename T>
concept permits_spectral_factory = requires(const fixed_symmetric matrix) { T::from_spectral_(matrix); };
template <typename T>
concept permits_braced_tag_construction = requires(const fixed_symmetric matrix) { T(matrix, {}); };
template <typename T>
concept permits_checked_tag_construction = requires(const fixed_matrix matrix) { T(matrix, checked); };

// the single-argument public constructor accepts a native dense matrix explicitly
static_assert(std::is_constructible_v<fixed_spd, const fixed_matrix&>);
// implicit conversion cannot silently trigger SPD validation
static_assert(!std::is_convertible_v<const fixed_matrix&, fixed_spd>);
// the removed checked-tag constructor is not retained as a compatibility overload
static_assert(!permits_checked_tag_construction<fixed_spd>);
// users cannot name the private tag that authorizes trusted construction
static_assert(!exposes_trusted_tag<fixed_spd>);
// users cannot invoke the private factory reserved for spectral primitives
static_assert(!permits_spectral_factory<fixed_spd>);
// an empty braced argument cannot bypass private constructor access
static_assert(!permits_braced_tag_construction<fixed_spd>);
...
static_assert(!permits_rvalue_assignment<fixed_spd>);
...
template <typename SPDType, typename MatrixType> void expect_spectral_oracles(const MatrixType& dense) {
    ...
    EXPECT_NO_THROW((SPDType(dynamic_matrix(restored))));
    ...
    EXPECT_NO_THROW((SPDType(dynamic_matrix(inverse_root))));
}
...
TEST(linear_algebra, spd_copy_and_checked_assignment_preserve_the_invariant) {
    ...
    fixed_spd point(initial);
    fixed_spd copied(point);
    fixed_spd assigned(reference_spd(2.0));
    assigned = point;
    ...
    dynamic_spd dynamic_point(two_by_two);
    dynamic_point.assign(dynamic_matrix(initial));
    ...
    dynamic_spd dynamic_assigned(two_by_two);
...
TEST(linear_algebra, spd_checked_construction_rejects_invalid_inputs_and_oversize_shapes) {
    ...
    EXPECT_THROW((dynamic_spd(empty)), std::invalid_argument);
    // rectangular input cannot initialize a positive-definite owner
    EXPECT_THROW((dynamic_spd(nonsquare)), std::invalid_argument);
    // a two-dimensional source cannot initialize a fixed three-dimensional owner
    EXPECT_THROW((fixed_spd(Matrix<double, 2, 2>({2.0, 0.0, 0.0, 2.0}))), std::invalid_argument);
    // unequal off-diagonal entries fail the symmetry check
    EXPECT_THROW((fixed_spd(asymmetric)), std::invalid_argument);
    // a negative diagonal entry makes this source indefinite and is rejected
    EXPECT_THROW((fixed_spd(indefinite)), std::domain_error);
    // infinite input is rejected before spectral validation
    EXPECT_THROW((fixed_spd(nonfinite)), std::invalid_argument);
    ...
    for (const double scale : {1.0e-150, 1.0e150}) {
        ...
        EXPECT_THROW((fixed_spd(scaled_indefinite)), std::domain_error);
    }
    ...
    EXPECT_THROW((dynamic_spd(too_large)), std::length_error);
...
TEST(linear_algebra, spd_numerical_positive_definiteness_uses_a_relative_threshold) {
    ...
    EXPECT_NO_THROW((fixed_spd(accepted)));
    ...
    EXPECT_THROW((fixed_spd(rejected)), std::domain_error);
}
...
TEST(linear_algebra, spd_spectral_results_preserve_the_numerical_invariant) {
    ...
    try {
        ...
        EXPECT_NO_THROW((fixed_spd(fixed_matrix(restored))));
...
TEST(linear_algebra, spd_frechet_differentials_handle_repeated_and_close_spectra) {
    ...
    const fixed_spd point(two_identity);
    ...
    close(1, 1) = 2.0 + gap;
    close(2, 2) = 5.0;
    const fixed_spd close_point(close);
...
TEST(linear_algebra, spd_frechet_differentials_are_inverse_and_match_finite_differences) {
    ...
    const dynamic_spd dynamic_point(dynamic_dense);
    ...
    const fixed_spd plus(dense + step * direction);
    const fixed_spd minus(dense - step * direction);
...
TEST(linear_algebra, spd_spectral_operations_own_results_from_safe_temporaries) {
    const fixed_matrix dense = reference_spd();
    const auto logarithm = matrix_log(fixed_spd(dense));
