...
concept permits_temporary_symmetric = requires {
    MatrixType {}.template as_symmetric<Lower>();
};

template <typename MatrixType>
concept permits_const_temporary_symmetric = requires(const MatrixType&& matrix) {
    std::move(matrix).template as_symmetric<Lower>();
};

template <typename MatrixType>
concept permits_expression_symmetric = requires(MatrixType& lhs, MatrixType& rhs) {
    (lhs + rhs).template as_symmetric<Lower>();
};
...
template <int StorageOrder> void check_dense_symmetric_views() {
    ...
    expect_matrix_near(
      temporary,
      matrix_type({2.0, 8.0, 14.0, 8.0, 10.0, 16.0, 14.0, 16.0, 18.0}));
    const auto temporary_upper = (dense + dense).template as_symmetric<Upper>();
    expect_matrix_near(
      temporary_upper,
      matrix_type({2.0, 4.0, 6.0, 4.0, 10.0, 12.0, 6.0, 12.0, 18.0}));
...
void check_packed_symmetric_contracts() {
    ...
    expect_matrix_near(
      coefficientwise,
      Matrix<double, 3, 3>({3.0, 4.0, 10.0, 4.0, 5.0, 7.0, 10.0, 7.0, 8.0}));

    SymmetricMatrix<double, Dynamic, Dynamic> dynamic_source(
    ...
    expect_matrix_near(
      overlap_destination,
      Matrix<double, 3, 3>({1.0, 2.0, 4.0, 2.0, 3.0, 5.0, 4.0, 5.0, 6.0}));
    ...
    EXPECT_THROW(
      static_cast<void>(
        SymmetricMatrixView<double, Dynamic, Dynamic>(static_cast<double*>(nullptr), 2, 2)),
      std::invalid_argument);
