...
concept permits_temporary_skew = requires {
    MatrixType {}.template as_skew_symmetric<Upper>();
};

template <typename MatrixType>
concept permits_const_temporary_skew = requires(const MatrixType&& matrix) {
    std::move(matrix).template as_skew_symmetric<Upper>();
};

template <typename MatrixType>
concept permits_expression_skew = requires(MatrixType& lhs, MatrixType& rhs) {
    (lhs + rhs).template as_skew_symmetric<Upper>();
};
...
template <typename MatrixType>
concept permits_temporary_skew_addition = requires {
    MatrixType {} + MatrixType {};
};

template <typename MatrixType>
concept permits_temporary_skew_scaling = requires {
    MatrixType {} * 2.0;
};

template <typename MatrixType>
concept permits_matrix_compound_multiply = requires(MatrixType& matrix) {
    matrix *= matrix;
};

template <typename MatrixType>
concept permits_scalar_compound_multiply = requires(MatrixType& matrix) {
    matrix *= 2.0;
};
...
template <int StorageOrder> void check_dense_skew_views() {
    ...
    expect_matrix_near(
      temporary_upper,
      matrix_type({0.0, 4.0, 6.0, -4.0, 0.0, 12.0, -6.0, -12.0, 0.0}));
    expect_matrix_near(
      temporary_lower,
      matrix_type({0.0, -8.0, -14.0, 8.0, 0.0, -16.0, 14.0, 16.0, 0.0}));
...
void check_dynamic_skew_contracts() {
    SkewSymmetricMatrix<double, Dynamic, Dynamic> dynamic(
    ...
    EXPECT_THROW(
      static_cast<void>(
        SkewSymmetricMatrixView<double, Dynamic, Dynamic>(static_cast<double*>(nullptr), 3, 3)),
      std::invalid_argument);
    EXPECT_THROW(
      static_cast<void>(SkewSymmetricMatrixView<double, 3, 3>(static_cast<double*>(nullptr))),
      std::invalid_argument);
