...
TEST(fem_pde_test, laplacian_dirichlet_neumann_2D) {
    // exact solution
...
// apply mixed boundary conditions
TEST(fem_pde_test, laplacian_dirichlet_neumann_2D_order2) {
    // exact solution
    auto solution_expr = [](SVector<2> x) -> double { return 1. + x[0]*x[0] + 2*x[1]*x[1]; };
    auto neumann_expr = [](SVector<2> x) -> double { return 4*x[1]; };
    auto forcing_expr = [](SVector<2> x) -> double { return -6.; };
...
// check error of approximated solution by an order 1 FEM within theoretical expectations in 3D
// apply mixed boundary conditions
TEST(fem_pde_test, laplacian_dirichlet_neumann_3D) {
    // exact solution
    auto solution_expr = [](SVector<3> x) -> double { return 1. + x[0]*x[0] + 2*x[1]*x[1] + 3*x[2]*x[2]; };
    ...
    //storing solution 
    // std::ofstream file("cube_DN.txt");    //it will be exported in the current build directory
    // if (file.is_open()){
    //     for(int i = 0; i < pde_.solution().rows(); ++i)
    //         file << pde_.solution()(i) << '\n';
    //     file.close();
...
TEST(fem_pde_test, space_time_dirichlet_neumann_2D) {
    ...
    auto neumann_expr = [](SVector<2> x, double t) -> double { 
        // if (x[0]==0 || x[0]==1) 
        return pi*std::sin(pi*x[0])*std::cos(pi*x[1])*std::exp(-t) * (1-2*x[0]); };
    ...
    for (size_t j=1; j<16; j++) {
        boundary_matrix(0 + 17*j, 0) = 1;
        boundary_matrix(16 + 17*j, 0) = 1;
    }
    ...
    // std::ofstream file("space_time_N_D_CNSI_P1_Tem3.txt");    //it will be exported in the current build directory
    // if (file.is_open()){
...
TEST(fem_pde_test, dirichlet_robin_1D) {
    // exact solution
...
TEST(fem_pde_test, space_time_dirichlet_robin_1D) {
    ...
    // non linear reaction h_(u)*u
    std::function<double(SVector<1>, SVector<1>)> h_ = [&](SVector<1> x, SVector<1> ff) -> double {return 1 - ff[0];};
    ...
    // std::ofstream file("space_time_R_D_ESI_P1_Tem3_1D.txt");    //it will be exported in the current build directory
    // if (file.is_open()){
    //     for(int i = 0; i < pde_.solution().col(M-1).rows(); ++i)
    //         file << pde_.solution().col(M-1)(i) << '\n';
    //     file.close();
...
TEST(fem_pde_test, space_time_dirichlet_robin_2D) {
    ...
    // considering the unit square,
    // we have Dirichlet boundary when y=0 and y=1 (upper and lower sides)
    // we have Robin boundary when x=0 and x=1 (left and right sides)
    DMatrix<short int> boundary_matrix = DMatrix<short int>::Zero(unit_square.mesh.n_nodes(), 1) ; // has all zeros
    for (size_t j=1; j<16; j++) {
    ...
    // std::ofstream file("space_time_R_D_ESI_P1_Tem3_2D.txt");    //it will be exported in the current build directory
    // if (file.is_open()){
    //     for(int i = 0; i < pde_.solution().col(M-1).rows(); ++i)
    //         file << pde_.solution().col(M-1)(i) << '\n';
    //     file.close();
...
TEST(fem_pde_test, deSolve_1D) {
    ...
    auto robin_expr = [&](SVector<1> x, double t) -> double { return F0;};

    Mesh1D domain(-0.01, 2.01, 101);
    ...
    // non linear reaction h_(u)*u
    std::function<double(SVector<1>, SVector<1>)> h_ = [&](SVector<1> x, SVector<1> ff) -> double {return r/(ff[0] + ks);};
    ...
    // std::ofstream file("solution_deSolve_prova.txt");    //it will be exported in the current build directory
    // if (file.is_open()){
    //     for (int j = 0; j < M; ++j) {
    //         // file << "M = " << j << "\n";
    //         for(int i = 1; i < pde_.solution().col(j).rows() - 1 ; ++i) {
    //             file << pde_.solution().col(j)(i) << '\n';
    //         }
    //         //file << "\n \n";
    //     }
    //     file.close();
