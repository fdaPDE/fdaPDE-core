...
TEST(simplex_test, triangle_2d) {
  ...
  EXPECT_TRUE(almost_equal(
    std::accumulate(
      t.boundary_begin(), t.boundary_end(), 0.0, [](double perimeter, auto& f) { return perimeter + f.measure(); }),
    0.5 + 0.8 + std::sqrt(0.5 * 0.5 + 0.8 * 0.8)));
  // extract face
  Simplex<2, 2>::BoundaryCellType f = *(t.boundary_begin() + 2);
...
TEST(simplex_test, triangle_3d) {
  ...
  EXPECT_TRUE(almost_equal(
    ...
    2.523402260893061));
  // extract face
  Simplex<2, 3>::BoundaryCellType f = *t.boundary_begin();
...
TEST(simplex_test, tetrahedron_3d) {
  ...
  EXPECT_TRUE(t.contains(t.boundary_begin()->barycenter()) == 2);         // point falls on face
  ...
  // extract face
  Simplex<3, 3>::BoundaryCellType f = *(t.boundary_begin() + 1);
  EXPECT_TRUE(
