...
TYPED_TEST(mesh_element_test, barycentric_coordinates_computation) {
    ...
    if constexpr (TestFixture::N == TestFixture::M) {
        ...
        HyperPlane<TestFixture::M, TestFixture::N> vs = e.hyperplane();
        EXPECT_FALSE(
    ...
    for (int i = 0; i < e.coords().cols(); ++i) {
        SVector<TestFixture::N> node = e.coord(i);
...
TYPED_TEST(mesh_element_test, mid_point_computation) {
    ...
    for (int i = 0; i < TestFixture::M; ++i) {
        double x = b[i];
        for (int j = i + 1; j < TestFixture::M + 1; ++j) {
            double y = b[j];
