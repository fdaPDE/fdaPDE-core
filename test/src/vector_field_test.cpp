...
#include "../../src/fields/dot_product.h"
...
TEST(VectorFieldTest, ExprToField) {
...
TEST(VectorFieldTest, DifferentDimensions) {
...
TEST(VectorFieldTest, UnaryNegation) {
  // define some field expression
  VectorField<2> vf_1;
  ...
  for(std::size_t i = 0; i < 2; ++i)
    EXPECT_DOUBLE_EQ( res_1[i], -vf_p[i]);
  ...
  for(std::size_t i = 0; i < 2; ++i)
    EXPECT_DOUBLE_EQ( res_2[i], -(2*vf_p[i]));
}
